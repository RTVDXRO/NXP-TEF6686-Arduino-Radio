...
{
  0x05, 0x1E, 0x5A, 0x01, 0x5A, 0x5A,     // Reset the device
  0x01, 100,                              // Wait 100ms
  0x03, 0x1c, 0x00, 0x00,                 // Clear
  0x03, 0x1C, 0x00, 0x74,                 // Set DSP_INIT bytes
  ...
  0x0D, 0x1B, 0xF0, 0x00, 0x10, 0x00, 0xF0, 0x00, 0xF0, 0x00, 0x70, 0x00, 0xD0, 0x08, // End of DSP_INIT bytes
  0x03, 0x1c, 0x00, 0x00, // Clear
  0x03, 0x1C, 0x00, 0x75, // Set LutByteValues
  ...
  0x17, 0x1B, 0x63, 0x69, 0x64, 0x3B, 0x66, 0x09, 0x66, 0xE7, 0x67, 0x0B, 0x67, 0x1A, 0x67, 0x29, 0x68, 0x87, 0x68, 0x99, 0x68, 0xA7, 0x68, 0xB2, // End of LutByteValues
  0x03, 0x1c, 0x00, 0x00,
  0x03, 0x14, 0x00, 0x01,                                           // Command ‘Start’ brings the device into ‘idle state’
  ...
  0x05, 0x40, 0x05, 0x01, 0x00, 0x01,                               // Command APPL_Activate brings the device into ‘active state’ = radio standby
  0x02, 0xFF, 100,                                                  // Wait 100 ms

  0x05, 0x20, 0x50, 0x01, 0x00, 0x01,
  0x05, 0x20, 0x56, 0x01, 0x03, 0xB6,
  ...
  0x07, 0x20, 0x17, 0x01, 0x00, 0x00, 0x03, 0xE8,                                                              // FM Set_NoiseBlanker           (1, 0, 1000)
  0x09, 0x20, 0x18, 0x01, 0x00, 0xD2, 0x00, 0xD2, 0x00, 0xFA,                                                  // FM Set_NoiseBlanker_Option    (1, 210, 210, 250)
  // 0x05, 0x20, 0x1F, 0x01, 0x01, 0xF4,                                                                          // FM Set_Deemphasis             (1, 500)
  0x11, 0x20, 0x26, 0x01, 0xFF, 0xEC, 0xFF, 0xE2, 0xFF, 0xD8, 0xFF, 0xCE, 0xFF, 0xC4, 0xFF, 0xC4, 0xFF, 0xC4,  // FM Set_LevelStep              (1, -20, -30, -40, -50, -60, -60, -60)
  0x05, 0x20, 0x27, 0x01, 0x00, 0x00,                                                                          // FM Set_LevelOffset            (1, 0)

  0x0B, 0x20, 0x28, 0x01, 0x00, 0x64, 0x01, 0x90, 0x00, 0x0A, 0x00, 0x14,                                      // FM Set_Softmute_Time          (1, 100, 400, 10, 20)
  0x0B, 0x20, 0x29, 0x01, 0x00, 0x01, 0x00, 0xD2, 0x00, 0x8C, 0x01, 0x04,                                      // FM Set_Softmute_ Mod          (1, 1, 210, 140, 260)
  0x09, 0x20, 0x2A, 0x01, 0x00, 0x01, 0x00, 0x64, 0x00, 0xDC,                                                  // FM Set_Softmute_Level         (1, 1, 100, 220)
  0x09, 0x20, 0x2B, 0x01, 0x00, 0x01, 0x01, 0x90, 0x03, 0xE8,                                                  // FM Set_Softmute_Noise         (1, 1, 400, 1000)
  0x09, 0x20, 0x2C, 0x01, 0x00, 0x01, 0x01, 0x90, 0x03, 0xE8,                                                  // FM Set_Softmute_Mph           (1, 1, 400, 1000)
  0x07, 0x20, 0x2D, 0x01, 0x00, 0x01, 0x00, 0x1E,                                                              // FM Set_Softmute_Max           (1, 1, 30)

  0x0B, 0x20, 0x32, 0x01, 0x00, 0xC8, 0x01, 0xF4, 0x00, 0x0A, 0x00, 0x50,                                      // FM Set_Highcut_Time           (1, 200, 500, 10, 80)
  0x0B, 0x20, 0x33, 0x01, 0x00, 0x01, 0x00, 0xFA, 0x00, 0x82, 0x01, 0xF4,                                      // FM Set_Highcut_ Mod           (1, 1, 250, 130, 500)
  0x09, 0x20, 0x34, 0x01, 0x00, 0x01, 0x00, 0xC8, 0x01, 0x2C,                                                  // FM Set_Highcut_Level          (1, 1, 200, 300)
  0x09, 0x20, 0x35, 0x01, 0x00, 0x01, 0x01, 0x2C, 0x00, 0xC8,                                                  // FM Set_Highcut_Noise          (1, 1, 300, 200)
  0x09, 0x20, 0x36, 0x01, 0x00, 0x01, 0x01, 0x2C, 0x00, 0xC8,                                                  // FM Set_HighCut_Mph            (1, 1, 300, 200)
  0x07, 0x20, 0x37, 0x01, 0x00, 0x01, 0x27, 0x10,                                                              // FM Set_HighCut_Max            (1, 1, 10000)
  0x07, 0x20, 0x38, 0x01, 0x00, 0x01, 0x3A, 0x98,                                                              // FM Set_Highcut_Min            (1, 1, 15000)
  0x07, 0x20, 0x39, 0x01, 0x00, 0x01, 0x00, 0x28,                                                              // FM Set_Lowcut_Max             (1, 1, 40)
  0x07, 0x20, 0x3A, 0x01, 0x00, 0x01, 0x00, 0x14,                                                              // FM Set_Lowcut_Min             (1, 1, 20)

  0x0B, 0x20, 0x3C, 0x01, 0x00, 0xC8, 0x07, 0xD0, 0x00, 0x14, 0x00, 0x50,                                      // FM Set_Stereo_Time            (1, 200, 2000, 20, 80)
  0x0B, 0x20, 0x3D, 0x01, 0x00, 0x01, 0x01, 0xA4, 0x01, 0x40, 0x03, 0xE8,                                      // FM Set_Stereo_Mod             (1, 1, 420, 320, 1000)
  0x09, 0x20, 0x3E, 0x01, 0x00, 0x01, 0x01, 0xCC, 0x00, 0xF0,                                                  // FM Set_Stereo_Level           (1, 1, 300, 300)
  0x09, 0x20, 0x3F, 0x01, 0x00, 0x01, 0x02, 0x58, 0x00, 0xB4,                                                  // FM Set_Stereo_Noise           (1, 1, 600, 180)
  0x09, 0x20, 0x40, 0x01, 0x00, 0x01, 0x02, 0x58, 0x00, 0xDC,                                                  // FM Set_Stereo_Mph             (1, 1, 600, 220)
  0x05, 0x20, 0x41, 0x01, 0x00, 0x01,                                                                          // FM Set_Stereo_Max             (1, 1)
  0x07, 0x20, 0x42, 0x01, 0x00, 0x00, 0x00, 0x3C,                                                              // FM Set_Stereo_Min             (1, 0, 60)

  0x0B, 0x20, 0x46, 0x01, 0x00, 0x78, 0x00, 0xFA, 0x00, 0x50, 0x00, 0x28,                                      // FM Set_StHiBlend_Time         (1, 120, 250, 80, 40)
  0x0B, 0x20, 0x47, 0x01, 0x00, 0x01, 0x00, 0xF0, 0x00, 0x78, 0x02, 0x9E,                                      // FM Set_StHiBlend_Mod          (1, 1, 240, 120, 670)
  0x09, 0x20, 0x48, 0x01, 0x00, 0x01, 0x01, 0xF4, 0x01, 0x2C,                                                  // FM Set_StHiBlend_Level        (1, 1, 500, 300)
  0x09, 0x20, 0x49, 0x01, 0x00, 0x01, 0x00, 0x50, 0x00, 0xA0,                                                  // FM Set_StHiBlend_Noise        (1, 1, 80, 160)
  0x09, 0x20, 0x4A, 0x01, 0x00, 0x01, 0x00, 0x50, 0x00, 0xA0,                                                  // FM Set_StHiBlend_Mph          (1, 1, 80, 160)
  0x07, 0x20, 0x4B, 0x01, 0x00, 0x01, 0x09, 0x60,                                                              // FM Set_StHiBlend_Max          (1, 1, 2400)
  0x07, 0x20, 0x4C, 0x01, 0x00, 0x01, 0x3A, 0x98,                                                              // FM Set_StHiBlend_Min          (1, 1, 15000)
  0x07, 0x20, 0x5A, 0x01, 0x00, 0x64, 0x00, 0x1E,                                                              // FM_Set_StBandBlend_Time       (1, 100, 30)
  0x0B, 0x20, 0x5B, 0x01, 0x05, 0xDC, 0x05, 0xDC, 0x05, 0xDC, 0x05, 0xDC,                                      // FM_Set_StBandBlend_Gain       (1, 1500, 1500, 1)
...
// XDR-GTK Antenna Switch option ( Ant n or S n or DX n ... etc )
...
{
  if (val == 0) {
    Set_Cmd(32, 11, 1, 920, 0);
    Set_Cmd(33, 11, 1, 1020);
  } else if (val == 1) {
    Set_Cmd(32, 11, 1, 900, 0);
    Set_Cmd(33, 11, 1, 1000);
  } else if (val == 2) {
    Set_Cmd(32, 11, 1, 880, 0);
    Set_Cmd(33, 11, 1, 980);
  } else {
    Set_Cmd(32, 11, 1, 860, 0);
    Set_Cmd(33, 11, 1, 960);
  }
}

void Set_RF_Gain(uint8_t val, uint8_t val1)
{
  if (val == 1 && val1 == 0) {
    Set_Cmd(32, 39, 1, 60);
    Set_Cmd(33, 39, 1, 30);
...
void loop()
{
  ...
  if ((millis() - timer) >= TIMER_INTERVAL)
  {
    ...
    if ((uQuality[2] > 1000) && (radio_mode == 0)) {
      uQuality[2] = 1000; // Shoots sometimes above 1000 but max is 1000 in datasheet,
    }
    if (radio_mode == 1) {
      uQuality[2] = uQuality[2] / 5; // for AM max 500% reset to 100%
    }
      ...
      switch (buff[0])
      {
        ...
        case 'Y':  // Audio volume scaler
          ...
          if (volume == 0) {
            ...
            // Set_Cmd(48, 10, 1, volume *6 - 570);  // variable volume control
            Set_Cmd(48, 11, 1, 0);      // unmute
          }
          Serial.print("Y");
        ...
        case 'T':  // Tune to a frequency (to do keep filter settings)
          ...
          if ((REG_FREQ >= 65000) && (REG_FREQ <= 108000)) {
            ...
            Serial.print('T');
            Serial.println(REG_FREQ);
            if (radio_mode == 0) {                       // from FM to AM then use filter 21 -> 4khz
              if (Filter_FM != 127) {
                Filter_FM = current_filter;
              }
              if (Filter_AM == 127) {
        ...
        case 'V':      // Set AM lna (in XDR-GTK enable Show antenna input alignment) (NO FM)
        ...
        /*
          case 'C':  // Xtreme DX threshold extender ( increased sensitivity with lower demodulation threshold - optional additional external circuit required )
          Lorem ipsum - see rdi.boards.net for more ...
        */
